...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div title="表单目录树" region="west" style="float:left;overflow:auto;width:17%;height:100%"
		showFilter="false"
		class="sub-sidebar" allowResize="false">
		<div class="form-control" id="jsp" style="height:100%;padding:20px 10px;">
			<ul id="form_tree" class="ztree" value="FOLDER_TREE_NAME" style="overflow:auto;height:100%" parentField="FOLDER_TREE_PARENT_ID" >
			</ul>
		</div>
	</div>
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/form/queryFormAndTableTree.action",//ajax的请求地址
				autoParam:["FOLDER_TREE_ID","level=lv"], //传递的参数 id lv（层级，最开始是0）
				dataFilter: filter
			},
			//双击节点时，是否自动展开父节点的标识
			view: { dblClickExpand: false,showLine: false,addDiyDom: addDiyDom}, 
		};
		var paramtabs_map = {FORM: [{title:'表单目录树',
							 url:'<%= basePath%>jsp/form/formcreate/formparamList.jsp'}]}		
		...
		function onClick(event, treeId, treeNode){  
			var main = document.getElementById("main");
				if(treeNode.TYPE=='FORM'){
					main.src = 'http://myehr430s:8080/myehr/jsp/form/formcreate/formColumnParams.jsp?formId='+treeNode.FOLDER_TREE_ID;
				}
		}; 
...
function setUrlParam(url,params){
	...
	function transmit(columnName){
		var objId = '${param.objId}';
		parent.$("#"+objId).val(columnName);
		closex();
	}
